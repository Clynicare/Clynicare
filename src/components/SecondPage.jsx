...
const SecondPage = React.memo(() => {
  ...
  // Toggle function to open/close description
  const toggleStep = useCallback((index) => {
    setOpenStep((prev) => (prev === index ? null : index)); // Close if it's already open
  }, []);
  ...
  return (
    <div className="h-auto px-5 md:px-[180px] py-10 md:py-[100px] flex flex-col md:flex-row gap-10 md:gap-[70px]">
      <motion.div
        ...
        className="leftpart flex flex-col items-center md:items-start"
      >
        <Image
          src="/images/doc.jpg"
          width={400}
      ...
      <motion.div className="rightpart w-full md:w-auto">
        <div className="content w-full md:w-[600px] flex flex-col gap-5 text-center md:text-left">
          ...
          <p className="font-bold text-lg md:text-xl text-gray-600 border-b-2 pb-4">
            With our user-friendly platform, accessing top-notch nursing care has never been easier.
          </p>
        </div>
            ...
            <div key={step.step} className="border-b-2 pb-4 flex flex-row gap-5 items-center">
              ...
              <div className="stepcontent w-full">
                <button
                  ...
                  className="text-lg md:text-2xl font-semibold w-full text-left"
                >
                  {step.title}
                </button>
                <div
                  className={`transition-all duration-300 ease-in-out overflow-hidden ${openStep === index ? "max-h-40 opacity-100" : "max-h-0 opacity-0"}`}
                >
